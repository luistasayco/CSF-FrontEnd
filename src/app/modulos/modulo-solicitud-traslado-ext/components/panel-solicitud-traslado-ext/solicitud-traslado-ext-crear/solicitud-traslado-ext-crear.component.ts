...
@Component({
  selector: 'app-solicitud-traslado-ext-crear',
  templateUrl: './solicitud-traslado-ext-crear.component.html',
  styleUrls: ['./solicitud-traslado-ext-crear.component.css']
})

export class SolicitudTrasladoExtCrearComponent implements OnInit, OnDestroy {
  ...
  clickGuardar(){
    ...
    this.solicitudTrasladoService.setSolicitudTrasladoRegistrar(value).subscribe(
      (result: IMensajeResultadoApi) =>{
        this.sessionStorage.setItemEncrypt('idsolicitudtraslado-ext', result["idRegistro"]);
