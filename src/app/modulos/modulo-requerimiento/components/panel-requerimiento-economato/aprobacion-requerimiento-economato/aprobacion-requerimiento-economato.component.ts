...
import { IRequerimiento, IRequerimientoAutorizar, IRequerimientoAnular, IRequerimientoAprobar } from '../../../models/requerimiento.interface';
...
@Component({
  selector: 'app-aprobacion-requerimiento-economato',
  templateUrl: './aprobacion-requerimiento-economato.component.html',
  styleUrls: ['./aprobacion-requerimiento-economato.component.css']
})
export class AprobacionRequerimientoEconomatoComponent implements OnInit {
  // Titulo del componente
  titulo = 'Aprobación - Requerimiento Economato';
  ...
  ngOnInit() {
    if (this.sessionStorage.getItem('busquedaRequerimientoEconomatoAprobacion')) {
        const bodyBusqueda = this.sessionStorage.getItem('busquedaRequerimientoEconomatoAprobacion');
  ...
  autorizarRequerimiento() {
    this.confirmationService.confirm({
      message: `Seguro Aprobar el requerimiento con el Id ${this.itemSeleccionado.idRequerimiento} ?`,
      header: 'Confirmación de Aprobación',
      icon: 'pi pi-info-circle',
      acceptLabel: 'Si',
      rejectLabel: 'No',
      accept: () => {
        const reqAprobar: IRequerimientoAprobar = {
        ...
        this.requerimientoService.aprobarRequerimiento(reqAprobar).subscribe(
  ...
  clickBusqueda() {
    ...
    const idRequerimientoEstado = 9;
    this.search(fechaIn, fechaFin, idUsuario, idRequerimientoEstado);
  }

  search(fechaIn, fechaFin, idUsuario, idRequerimientoEstado) {
    console.log('PARAMETROS DE BUSQUEDA EN PANEL DE REQUERIMIENTOS:', fechaIn, fechaFin, idUsuario, idRequerimientoEstado);
      ...
      .pipe(
        map((resp) => {
          if (resp.length === 0) {
            ...
            this.sessionStorage.setItem('busquedaRequerimientoEconomatoAprobacion', bodyBusqueda);
          }
        })
      )
      .subscribe();
